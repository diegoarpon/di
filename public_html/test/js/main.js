...
function randomizeBrandCreationLogos() {
  const tilesToRandomize = Array.from(document.querySelectorAll('#brand-creation .row .tile img:not(.tile-end img)'))
    .filter(img => !img.closest('.carousel') && !img.closest('.tile-xl'));
  
  const shuffledLogos = [...brandCreationLogos.filter(logo => !logo.includes('zoom'))]
...
function addPanelsToTiles() {
  document.querySelectorAll('#brand-creation .tile:not(.carousel):not(.tile-xl)').forEach(tile => {
    ...
    const content = getProjectContent(filename, currentLang) || { name: 'Project', industry: 'Brand Design', year: '2023' };
...
function addCol4Panels() {
  document.querySelectorAll('#brand-creation .tile, #brand-creation .tile-xl').forEach(tile => {
    tile.querySelectorAll('.brand-hover-overlay, .brand-hover-text').forEach(el => el.remove());
    
    const img = tile.querySelector('img');
    if (!img || !img.src) return;
    
    const filename = img.src.split('/').pop();
    const content = getProjectContent(filename, currentLang) || { name: 'Project', industry: 'Brand Design' };
...
function showContent(category) {
  ...
  const activeBtn = document.querySelector(`[onclick="showContent('${category}')\"]`);
...
function toggleSidebar() {
  ...
  const fixedToggle = document.querySelector('div[style*="position: fixed"] .mobile-menu-toggle');
